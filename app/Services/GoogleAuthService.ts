...
export const useGoogleAuth = () => {
  ...
  const signInWithGoogle = async () => {
    try {
      ...
      if (!tokens.idToken) {
        throw new Error("No se pudo obtener un token de identificación de Google");
      }
      ...
      if (error.code) {
        switch(error.code) {
          ...
          case statusCodes.PLAY_SERVICES_NOT_AVAILABLE:
            errorMessage = 'Google Play Services no está disponible o actualizado';
            break;
          default:
      ...
      if (error && typeof error === 'object') {
        try {
          ...
          console.error('[GoogleAuth] Error with additional properties that cannot be stringified');
        }
      }
